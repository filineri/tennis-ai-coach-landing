...
const formatDateIt=(iso)=>{const [y,m,d]=clean(iso,10).split('-');return y&&m&&d?`${d}/${m}/${y}`:clean(iso,10);};
...
function buildTimeline(proposal){
 ...
 for(const e of proposal.events||[]){
  ...
  rows.push({id:`${e.id}-logistics`,date:addDays(e.date,-7),label:e.commuteEligible?`Verifica tragitto e rientro giornaliero · ${e.name}`:`Blocca viaggio e alloggio flessibili · ${e.name}`,status:'PENDING',eventId:e.id});
...
function checklist(proposal){
 return [{id:'selected',label:'Tour scelto',status:'DONE'},...proposal.events.flatMap(e=>[
  ...
  {id:`${e.id}-travel`,label:e.commuteEligible?`Trasferimento giornaliero · ${e.city}`:`Viaggio / hotel · ${e.city}`,status:'PENDING'},
